...
**Core Pipeline**: `Source Document → Create Project → Template Option → Strategist → [Image_Generator / Web Image Companion] → Executor → Post-processing → Export`
...
> **Companion routing rule**: If the user explicitly wants browser-based generation, lacks usable image API keys, or already has logged-in memberships on Gemini / Nano Banana / ChatGPT / Grok / Doubao, prefer the companion skill `ppt-master-web-image-pipeline` when it is installed. The companion skill remains part of Step 5 and must hand the project back only after `images/` is ready for Executor.

Read `references/image-generator.md`

1. Extract all images with status "pending generation" from the design spec
2. Decide the execution branch:
   - **CLI / API branch**: use `${SKILL_DIR}/scripts/image_gen.py` when the user has working API credentials
   - **Web companion branch**: invoke `ppt-master-web-image-pipeline` when the user prefers logged-in web tools over API keys
3. Generate prompt document → `<project_path>/images/image_prompts.md`
4. Generate images through the chosen branch:
   ```bash
   python3 ${SKILL_DIR}/scripts/image_gen.py "prompt" --aspect_ratio 16:9 --image_size 1K -o <project_path>/images
   ```
   When using the web companion branch, require these additional Step 5 deliverables before continuing:
   - `<project_path>/images/web_generation_manifest.json`
   - cleaned image assets saved under the exact filenames from the Image Resource List
   - `design_spec.md` image rows updated from `Pending` to `Generated` only after files exist locally
...
- [x] Companion web manifest created (if web branch used)
- [x] Image resource statuses updated
```
