...
When the user does not have usable image API keys but does have logged-in memberships on web image tools, Step 5 may be completed through the companion skill `ppt-master-web-image-pipeline`. In that branch, Image_Generator still owns the prompt quality, filename discipline, verification, and handoff back to Executor.
...
| Web manifest | `project/images/web_generation_manifest.json` | Required when using the companion web-generation branch |
...
> - Prefer **Method 1 / Method 2** when API generation is configured and acceptable to the user
> - Prefer **Method 3** when the user wants browser-based generation, does not have API keys, or explicitly requests Gemini / Nano Banana / ChatGPT / Grok / Doubao web workflows
> - Use **Method 4 / Method 5** only when the companion skill is unavailable or the user explicitly wants a more manual flow
...
#### Method 3: Companion Skill for Logged-in Web Image Tools (Recommended when no API key is available)

Use `ppt-master-web-image-pipeline` when installed. It is the preferred Step 5 route for browser-based image generation because it preserves PPT Master's filenames, prompt document format, manifest, and verification flow.
...
**Companion workflow expectations**:

1. Read `design_spec.md` and extract pending rows from the Image Resource List
2. Build `project/images/image_prompts.md`
3. Build `project/images/web_generation_manifest.json`
4. Reuse the user's logged-in browser sessions to generate one image at a time
5. Download or capture the clean asset under the exact filename required by the Image Resource List
6. Remove visible platform chrome, watermarks, and corner badges; regenerate instead of heavily editing if the mark overlaps the main subject
7. Verify the file exists in `project/images/`, then update the corresponding status to `Generated`

**Invocation guidance**:
- If the skill is available in `$CODEX_HOME/skills` or `~/.codex/skills`, explicitly route Step 5 to `ppt-master-web-image-pipeline`
- Keep the deck visually coherent by reusing the same provider for most images unless a specific prompt needs a different site
- Route precise labeled charts and strict technical diagrams back to Executor or manual SVG construction instead of web image sites

#### Method 4: Gemini Web Interface (Legacy direct route)
...
> Prefer Method 3 over this direct route whenever the companion skill is installed, because Method 3 also produces the manifest and standardized verification artifacts.
...
- When the web companion branch is used, confirm `images/web_generation_manifest.json` exists and reflects the selected providers
- Reject images with visible platform UI chrome, watermarks, or corner badges; clean minor edge marks or regenerate as needed
