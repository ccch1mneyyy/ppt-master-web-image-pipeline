...
> **Pipeline handoff**: When C) AI generation is selected, after outputting the design spec, route Step 5 according to the user's generation path. Use the standard Image_Generator / CLI route when API credentials are available; when the user prefers logged-in web tools or has memberships but no API keys, hand Step 5 to the companion skill `ppt-master-web-image-pipeline` if installed. Once images are collected in `images/` and statuses are updated, proceed to Executor.
